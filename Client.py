...
def Manual_key_generation(string):
    ...
    e = int(e)
    phi_n = (p-1)*(q-1)
    if (not(sympy.isprime(p)) or not(sympy.isprime(q)) or (RSA.GCD(phi_n, e) != 1) or e >= phi_n or e <= 1 or p == q or p == 1 or q == 1 or isnan(p) or isnan(q)):
...
while p == 1:
    Auto_Or_Manual = input(
        "Do you want key generation to be (1) automatic or (2) manual?\n")
    if int(Auto_Or_Manual) == 1:
        e, d, n, p, q = Auto_key_generation()
    elif int(Auto_Or_Manual) == 2:
        e, d, n, p, q = Manual_key_generation(
            input("Please enter p,q,e separated by comma:\n"))
